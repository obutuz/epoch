...
apply_signed_txs_common(Miner, SignedTxs0, Trees0, Height, ConsensusVersion, Strict) ->
    ...
    case apply_txs_on_state_trees(SignedTxs, Trees1, Height, ConsensusVersion, Strict) of
        {ok, SignedTxs1, Trees2} ->
            ...
            Trees3 = grant_fee_to_miner(Miner, Trees2, TotalFee, Height),
...
-spec grant_fee_to_miner(pubkey(), trees(), non_neg_integer(), height()) ->
                                trees().
grant_fee_to_miner(MinerPubkey, Trees0, TotalFee, Height) ->
    {ok, Trees1} = ensure_account_at_height(MinerPubkey, Trees0, Height),
    AccountsTrees1 = accounts(Trees1),

    {value, Account1} = aec_accounts_trees:lookup(MinerPubkey, AccountsTrees1),
    {ok, Account} = aec_accounts:earn(Account1, TotalFee, Height),
